...
<div class="min-h-screen flex items-center justify-center bg-[#f6f8f8] py-12 px-4 sm:px-6 lg:px-8 animate-fade-in relative overflow-hidden">
    ...
    <div class="max-w-md w-full space-y-8 glass-card p-10 rounded-3xl shadow-2xl border border-white/20 backdrop-blur-md relative z-10 hover:shadow-3xl transition-shadow duration-500 animate-scale-in">
        ...
        <div class="text-center">
            ...
            <h2 class="mt-6 text-3xl font-extrabold text-[#2E282A]">Reset Password</h2>
        ...
        <form action="{{ route('user.password.update') }}" method="POST" class="mt-8 space-y-6">
            ...
            <div class="text-center mt-6">
                @if(request()->is('seller*'))
                    <a href="{{ route('seller.login') }}" class="group inline-flex items-center gap-2 text-sm font-medium text-[#718991] hover:text-[#13c8ec] transition-colors duration-300">
                        <i class="fas fa-arrow-left group-hover:-translate-x-1 transition-transform duration-300"></i>
                        Back to Seller Login
                    </a>
                @else
                    <a href="{{ route('user.login') }}" class="group inline-flex items-center gap-2 text-sm font-medium text-[#718991] hover:text-[#13c8ec] transition-colors duration-300">
                        <i class="fas fa-arrow-left group-hover:-translate-x-1 transition-transform duration-300"></i>
                        Back to Login
                    </a>
                @endif
